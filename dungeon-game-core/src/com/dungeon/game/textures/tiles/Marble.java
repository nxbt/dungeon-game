		...
		for(int i = 0; i < 32; i++){
			for(int k = 0; k < 32; k++){
				...
				texMap.setColor(new Color(mortarColor.r*0.8f*(0.9f+num*0.1f), mortarColor.g*0.8f*(0.9f+num*0.1f), mortarColor.b*0.8f*(0.9f+num*0.1f), 1));
					...
					float num = (0.7f+ MathUtils.noise2d(seed, newCurX, newCurY, 2)*0.3f)*MathUtils.perturbedSinNoise2d(MathUtils.getRandomFromSeedAndCords(seed, x, y).nextInt(1000), newCurX, newCurY, 7, 6, 75);
					texMap.setColor(new Color(tileColor.r*(0.8f+0.4f*num), tileColor.g*(0.8f+0.4f*num), tileColor.b*(0.8f+0.4f*num), 1));
				}
